...
const PatternGame = ({ sequence, answer, onComplete }) => {
  const [input, setInput] = useState("");
  const [feedback, setFeedback] = useState(null);

  const handleCheck = () => {
    if (input.trim() === String(answer)) {
      setFeedback("Correct!");
    } else {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-lg mx-4">
      <h3 className="text-xl font-bold text-slate-800 mb-2">Pattern Game</h3>
      <p className="text-slate-600 text-base mb-4 text-center max-w-xs">
        What comes next in the sequence?<br />
        <span className="font-mono text-lg">{sequence.join(", ")}, ?</span>
      </p>
